...
/** ตัด/คืนสต็อกจาก order_items ไปที่ products.stock_* */
function update_inventory(PDO $pdo, int $orderId, string $mode='decrease'): void {
  $mode = ($mode==='increase') ? 'increase' : 'decrease';
  $st = $pdo->prepare("
    SELECT oi.product_id, UPPER(TRIM(COALESCE(oi.size,''))) AS size, COALESCE(oi.qty,0) AS qty
    FROM order_items oi
    WHERE oi.order_id = ?
  ");
  $st->execute([$orderId]);
  ...
  try {
    foreach ($rows as $r) {
      ...
      $sql = "UPDATE products SET {$col} = GREATEST({$col} + :delta, 0) WHERE id = :pid";
      $upd = $pdo->prepare($sql);
      $upd->execute([':delta'=>$delta, ':pid'=>$pid]);
    }
    $pdo->commit();
  } catch (Throwable $e) {
